/* ============================================================
   MAIN.JS — Vinícius Rafael Fotografia  (v2.0)
   Módulos:
     1.  SPA         — navegação entre views
     2.  Hero        — slideshow
     3.  Galeria     — carregamento de imagens
     4.  Lightbox    — visualização full-screen
     5.  Chart       — gráfico radar
     6.  Navbar      — efeito ao rolar
     7.  Frases      — frases rotativas (fix localStorage + fallback)
     8.  Animações   — scroll reveal suave
     9.  Mobile      — menu mobile corrigido
   ============================================================ */
...
const app = {
  ...
  _makeItem(src, gridRef) {
    ...
    item.className = 'masonry-item overflow-hidden rounded-sm shadow-2xl loading-skeleton min-h-[200px] reveal';
  ...
  openGallery(theme, customData = null) {
    ...
    hide('pre-wedding-section'); hide('ceremony-section'); hide('wedding-nav-buttons');
    ...
    if (theme === 'wedding-detail') {
      ...
      show('wedding-nav-buttons'); show('pre-wedding-section'); show('ceremony-section');
  ...
  renderChart() {
    ...
    new Chart(ctx.getContext('2d'), {
      ...
      data: {
        ...
        datasets: [{ data: [100, 100, 95, 88, 98, 100], backgroundColor: 'rgba(197, 163, 115, 0.2)', borderColor: '#c5a373', pointBackgroundColor: '#0a0c0a', borderWidth: 2 }],
      },
      options: {
        responsive: true, maintainAspectRatio: false,
        scales: { r: { grid: { color: 'rgba(0,0,0,0.08)' }, ticks: { display: false }, pointLabels: { color: '#2c2f2e', font: { size: 11 } } } },
...
const lightbox = {
  el: null, imgEl: null, allImgs: [], currentIndex: 0,
  ...
  close() { this.el.classList.remove('lb-active'); document.body.style.overflow = ''; this.imgEl.src = ''; },
  prev()  { this.currentIndex = (this.currentIndex - 1 + this.allImgs.length) % this.allImgs.length; this._load(this.allImgs[this.currentIndex].src); },
  next()  { this.currentIndex = (this.currentIndex + 1) % this.allImgs.length; this._load(this.allImgs[this.currentIndex].src); },
  ...
  _load(src) {
    ...
    const tmp   = new Image();
    tmp.src     = src;
    tmp.onload  = () => { this.imgEl.src = src; this.imgEl.style.opacity = '1'; loader.style.display = 'none'; };
...
function initMobileMenu() {
  ...
  btn.addEventListener('click', (e) => {
    ...
    if (icon) { icon.className = isHidden ? 'fas fa-bars text-2xl' : 'fas fa-times text-2xl'; }
  });
...
/* ── Frases rotativas — sem localStorage (corrigido) ─────── */
